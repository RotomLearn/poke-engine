...
pub const DEFAULT_TOP_K_MOVES: usize = 13; // Default number of top moves to consider
const BATCH_SIZE: usize = 64; // Target batch size for policy evaluations

// Sigmoid function to normalize heuristic values between 0 and 1
...
// Policy Network - Only uses policy head from joint network
...
impl PolicyNetwork {
    pub fn new(model_path: &str, device: Device) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        Ok(PolicyNetwork { model, device })
    }

    pub fn evaluate_batch(&self, states: &[State]) -> (Vec<Vec<f32>>, Vec<Vec<f32>>) {
        ...
        // Extract policy from the combined tensor (ignore value output)
        ...
        (policies_s1, policies_s2)
    }

    pub fn evaluate(&self, state: &State) -> (Vec<f32>, Vec<f32>) {
        ...
        // Extract only policy from combined output (ignoring value)
        let policy_probs = combined_output.slice(1, 0, 3191, 1);
...
impl Node {
    fn new(
        ...
        let pruned_s1 = prune_to_top_k_moves(s1_options_with_policy, top_k_moves);
        let pruned_s2 = prune_to_top_k_moves(s2_options_with_policy, top_k_moves);
        ...
        Node {
            ...
            value_sum: 0.0,
            heuristic_value: evaluate(state),
        }
    }
    ...
    pub fn maximize_puct_value_aware_for_side(&self, side_map: &[MoveNode]) -> usize {
        let mut choice = 0;
        let mut best_score = f32::MIN;

        // Get position value estimate (normalized to 0-1)
        let position_value = sigmoid(self.heuristic_value);
    ...
    pub unsafe fn selection(&mut self, state: &mut State) -> (*mut Node, usize, usize) {
        ...
        // Use value-aware PUCT for selection
        let s1_mc_index = self.maximize_puct_value_aware_for_side(&self.s1_options);
        let s2_mc_index = self.maximize_puct_value_aware_for_side(&self.s2_options);
    ...
    pub unsafe fn expand(
        ...
        // Batch evaluate all states for policy only
        let (policies_s1, policies_s2) = network.evaluate_batch(&states);

        // Create nodes with the evaluated results
        let mut this_pair_vec = Vec::with_capacity(new_instructions.len());

        for ((instructions, state), (policy_s1, policy_s2)) in new_instructions
            .into_iter()
            .zip(states.into_iter())
            .zip(policies_s1.into_iter().zip(policies_s2.into_iter()))
        {
            let (s1_options, s2_options) = state.get_all_options();
            ...
            // Use heuristic evaluation for the value instead of neural network
            new_node.heuristic_value = evaluate(&state);
        ...
        // Return sigmoid-transformed heuristic value
        let heuristic_value = sigmoid((*new_node_ptr).heuristic_value);
        (new_node_ptr, heuristic_value)
    }
    ...
    pub unsafe fn backpropagate(&mut self, value: f32, state: &mut State) {
        ...
        parent_s2_movenode.total_score += 1. - value;
...
fn prune_to_top_k_moves(
    moves_with_policy: Vec<(MoveChoice, f32)>,
    top_k: usize,
) -> Vec<(MoveChoice, f32)> {
    if moves_with_policy.len() <= top_k {
    ...
    let mut pruned = sorted_moves
        .into_iter()
        .take(top_k) // Use parameter instead of constant
        .collect::<Vec<_>>();
...
fn do_mcts_batch(
    ...
    if eval_batch.is_ready() {
        process_policy_evaluation_batch(eval_batch, network);
    }
}

fn process_policy_evaluation_batch(batch: &mut JointEvaluationBatch, network: &PolicyNetwork) {
    if batch.is_empty() {
        return;
    }

    // Evaluate all states in batch for policy only
    let (policies_s1, policies_s2) = network.evaluate_batch(&batch.states);
    ...
    for i in 0..batch.states.len() {
        ...
        let (s1_move_index, s2_move_index) = unsafe {
            let s1_idx = (*node_ptr).maximize_puct_value_aware_for_side(&(*node_ptr).s1_options);
            let s2_idx = (*node_ptr).maximize_puct_value_aware_for_side(&(*node_ptr).s2_options);
            (s1_idx, s2_idx)
        };
        ...
        // Calculate value using heuristic evaluation
        let heuristic_value = unsafe { sigmoid((*new_node).heuristic_value) };

        // Backpropagate
        unsafe {
            (*new_node).backpropagate(heuristic_value, &mut state);
        }
    }
...
pub struct MctsResultPN {
    pub s1: Vec<MctsSideResultPN>,
    pub s2: Vec<MctsSideResultPN>,
    pub iteration_count: i64,
    pub max_depth: usize,
}

// Non-batched MCTS with Policy Network guidance and heuristic evaluation
// pub fn perform_mcts_pn(
//     state: &mut State,
//     s1_options: Vec<MoveChoice>,
//     s2_options: Vec<MoveChoice>,
//     max_time: Duration,
//     network: Arc<PolicyNetwork>,
// ) -> MctsResultPN {
//     // Get initial policy and all legal moves
//     let (policy_s1, policy_s2) = network.evaluate(state);
...
// Batched MCTS with Policy Network guidance and heuristic evaluation
pub fn perform_mcts_pn_batched(
    state: &mut State,
    s1_options: Vec<MoveChoice>,
    s2_options: Vec<MoveChoice>,
    max_time: Duration,
    ...
    while start_time.elapsed() < max_time {
        ...
        if !eval_batch.is_empty() {
            process_policy_evaluation_batch(&mut eval_batch, &network);
        }
