...
use crate::state::{MoveChoice, State, SideReference};
...
impl Node {
    ...
    fn custom_eval(&mut self, state: &mut State) -> f32 {
        ...
        unsafe {
            ...
            // Restore state in one pass
            for instructions in instructions_to_reverse.into_iter().rev() {
    ...
    pub fn rollout(&mut self, state: &mut State, root_eval: &f32) -> f32 {
        ...
        if battle_is_over == 0.0 {
            let eval = self.custom_eval(state);
